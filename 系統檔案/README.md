...
4. 唯讀登入永豐驗證金鑰，成功後立即登出。
5. 註冊 Windows 週一至週五 08:25 自動錄製排程。
6. 再查詢一次排程，確認它確實存在。
...
Windows 工作排程器的 S4U 背景工作階段執行，不需使用者登入，也不儲存
Windows 密碼；之後由 PowerShell 以 `Start-Process -WindowStyle Hidden`
啟動服務，因此不會跳出錄製終端機。
